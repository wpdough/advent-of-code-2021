...
class Line {
    ...
    findSlope(): number {
        let rise: number = Math.abs(this.a.y - this.b.y);
        let run: number = Math.abs(this.a.x - this.b.x);
        return rise / run;
    }
    ...
    findPoints(): Point[] {
        ...
        let points: Point[] = [];
        let distance: number = Math.abs(this.a.y - this.b.y);
        let rise: number = (this.a.y - this.b.y) / distance * -1;
        let run: number = (this.a.x - this.b.x) / distance * -1;

        let origin: Point = this.a;
        for (let n = 0; n < distance + 1; n++) {
            let point: Point = new Point(origin.x + run * n, origin.y + rise * n);
            points.push(point);
        }
...
let lines: Line[] = input.map(lineText => new Line(lineText))
    .filter(line => line.isHorizontalOrVertical() || line.isDiagonal());
